...
function initEventListeners() {
  ...
  // 자동 키워드 수집
  document.getElementById('btnAutoGenerate').addEventListener('click', handleAutoGenerate);
  document.getElementById('btnLoadPopularSearches').addEventListener('click', handleLoadPopularSearches);
...
async function handleKakaoSearch() {
  ...
  try {
    ...
    // 검색 로그 저장 (인기 검색어 분석용)
    await logSearch(query, data.places.length);
...
async function handleKeywordSearch(keyword) {
  ...
  try {
    ...
    // 검색 로그 저장 (인기 검색어 분석용)
    await logSearch(keyword, data.places.length);
...
async function handleAutoGenerate() {
  ...
  const statusDiv = document.getElementById('autoGenerateStatus');

  btn.disabled = true;
  statusDiv.className = 'auto-status loading';
  statusDiv.style.display = 'block';
  statusDiv.textContent = '⏳ 자동 키워드 생성 중... (최대 72개 검증)';

  try {
    const response = await fetch('/api/auto-generate-keywords');

    if (!response.ok) {
      throw new Error('자동 생성 API 호출 실패');
    }
    ...
    statusDiv.innerHTML = `
      ✅ ${result.total}개의 유효한 키워드를 찾았습니다!
      <div class="auto-keywords-grid" id="autoGeneratedKeywords"></div>
    `;

    // 생성된 키워드 표시
    const grid = document.getElementById('autoGeneratedKeywords');
    grid.innerHTML = result.keywords.map((kw, idx) => `
      <div class="auto-keyword-item">
        <label class="auto-keyword-label">
          <input type="checkbox" data-keyword='${JSON.stringify(kw)}' checked>
          ${kw.emoji} ${kw.keyword}
          <span class="auto-keyword-count">${kw.result_count}개</span>
        </label>
      </div>
    `).join('');
    ...
    addBtn.innerHTML = `
      <button type="button" class="btn-primary" onclick="addSelectedAutoKeywords()">
        선택된 키워드 추가하기
      </button>
    `;
    ...
    console.error('Auto-generate keywords failed:', error);
    statusDiv.className = 'auto-status error';
    statusDiv.textContent = '❌ 자동 생성 실패: ' + error.message;
  } finally {
    btn.disabled = false;
  }
}
...
// 방법 B: 인기 검색어 분석
async function handleLoadPopularSearches() {
  const btn = document.getElementById('btnLoadPopularSearches');
  const listDiv = document.getElementById('popularSearchesList');

  btn.disabled = true;
  listDiv.innerHTML = '<div class="auto-status loading">📊 인기 검색어 분석 중...</div>';
  listDiv.style.display = 'block';

  try {
    const response = await fetch('/api/popular-searches?days=30&minResults=5');

    if (!response.ok) {
      throw new Error('인기 검색어 API 호출 실패');
    }
    ...
    if (!result.keywords || result.keywords.length === 0) {
      listDiv.innerHTML = `
        <div class="auto-status" style="background: #fff3cd; color: #856404;">
          ℹ️ 아직 충분한 검색 데이터가 없습니다. 검색을 더 사용해주세요!
        </div>
      `;
      return;
    }

    listDiv.innerHTML = result.keywords.map(kw => `
      <div class="popular-search-item">
        <div class="popular-search-info">
          <span class="popular-search-emoji">${kw.emoji}</span>
          <span class="popular-search-text">${kw.keyword}</span>
          <span class="popular-search-meta">${kw.search_count}회 검색</span>
        </div>
        <button class="btn-add-to-keywords" onclick="addSingleKeyword('${kw.keyword}', '${kw.emoji}', '${kw.category}')">
          추가
        </button>
      </div>
    `).join('');
    ...
    console.error('Load popular searches failed:', error);
    listDiv.innerHTML = `
      <div class="auto-status error">
        ❌ 인기 검색어 로딩 실패: ${error.message}
      </div>
    `;
  } finally {
    btn.disabled = false;
...
// 자동 생성된 키워드 중 선택된 것들 추가
window.addSelectedAutoKeywords = async function() {
  const checkboxes = document.querySelectorAll('#autoGeneratedKeywords input[type="checkbox"]:checked');
  const keywords = Array.from(checkboxes).map(cb => JSON.parse(cb.dataset.keyword));

  if (keywords.length === 0) {
    showMessage('추가할 키워드를 선택해주세요', 'error');
    return;
  }
  ...
  try {
    ...
    showMessage(`${keywords.length}개의 키워드가 추가되었습니다!`, 'success');
    ...
    // 상태 초기화
    document.getElementById('autoGenerateStatus').innerHTML = '';

  } catch (error) {
    console.error('Failed to add auto-generated keywords:', error);
    showMessage('키워드 추가 실패: ' + error.message, 'error');
  }
};

// 단일 키워드 추가 (인기 검색어에서)
window.addSingleKeyword = async function(keyword, emoji, category) {
  try {
    const client = window.supabaseClient || await travelDB.getClient();
    ...
    if (existing) {
      showMessage('이미 등록된 키워드입니다', 'error');
      return;
    }
    ...
    console.error('Failed to add single keyword:', error);
    showMessage('키워드 추가 실패: ' + error.message, 'error');
  }
};

// 검색 로그 저장 (인기 검색어 분석용)
async function logSearch(query, resultCount) {
  // 로그인하지 않은 경우 기록하지 않음
  ...
  try {
    ...
    const { error } = await client
      .from('search_logs')
      .insert({
        user_id: authClient.currentUser.id,
        search_query: query,
    ...
    if (error) {
      ...
      if (error.code === '42P01' || error.message.includes('relation')) {
        console.log('ℹ️ search_logs 테이블이 없습니다. 인기 검색어 분석을 사용하려면 add-search-logs.sql을 실행하세요.');
        return;
      }
      throw error;
    }

    console.log(`📊 검색 로그 저장됨: "${query}" (결과 ${resultCount}개)`);
  } catch (error) {
    console.error('Failed to log search:', error);
    // 검색 로그 실패는 사용자 경험에 영향을 주지 않도록 조용히 처리
  }
}
